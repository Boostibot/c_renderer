...
typedef enum Lpf_Error {
    ...
    LPF_ERROR_COLLECTION_MULTIPLE_TYPES,
...
isize lpf_read_line(String source, Lpf_Entry* parsed);
    ...
    switch(entry.kind)
    {
        ...
        case LPF_KIND_COLLECTION_END: {
            ...
            else
            {
                if(had_types > 2)
                {
                    entry.error = LPF_ERROR_COLLECTION_MULTIPLE_TYPES;
                    goto line_end;
                }
...
void lpf_write_line_custom(String_Builder* builder, Lpf_Entry entry, const Lpf_Write_Options* options_or_null)
{
    ...
    if(options.max_value_size == 0)
        options.max_value_size = INT64_MAX;
    
    if(options.max_comment_size == 0)
        options.max_comment_size = INT64_MAX;
    
    if(options.set_escapes == false)
    {
        options.type_escape = '_';
        options.label_escape = '_';
    ...
    if(entry.kind == LPF_KIND_COMMENT)
    {
        ...
        for(isize i = 0; i < comment_segments.size; i++)
        {
            ...
            continuation.kind = LPF_KIND_COMMENT;
            ...
            lpf_write_line_unescaped(builder, continuation, downstream_options);
        }

    }
    else if(entry.kind == LPF_KIND_COLLECTION_END || entry.kind == LPF_KIND_COLLECTION_START)
    {
        Lpf_Entry continuation = {0};
...
const char* lpf_error_to_string(Lpf_Error error)
{
    switch(error)
    {
        ...
        case LPF_ERROR_COLLECTION_MULTIPLE_TYPES: return "LPF_ERROR_COLLECTION_MULTIPLE_TYPES";
    }
}
