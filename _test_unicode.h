...
void test_unicode_utf8_to_utf16()
{
    ...
    //Invalid 2 Octet Sequence "\xc3\x28"
    {
        ...
        TEST_MSG(unicode_codepoint_from_utf8("a") == unicode_codepoint_from_ascii('a'), "ascii should work correctly");
        TEST_MSG(unicode_codepoint_from_utf8("/") == unicode_codepoint_from_ascii('/'), "ascii should work correctly");
        TEST_MSG(unicode_codepoint_from_utf8("a") == unicode_codepoint_from_utf8("az"), "should interpret only one codepoint");
        TEST_MSG(unicode_codepoint_from_utf8("č") == unicode_codepoint_from_utf8("čž"), "should interpret only one codepoint");
...
void test_unicode_single_utf8_to_utf16(Test_Unicode_Compare compare, const char* input, const wchar_t* expected_output, Test_Unicode_Fail_At fail_at, codepoint_t replacement)
{
    ...
    TEST_MSG(converted != NULL, "OUT OF MEMORY");
    ...
    TEST_MSG(are_equal == should_be_equal, "outputs dont match (or match when they shouldnt!)"
        "\ninput:     %s"
        "\nexpected:  %ls"
...
void test_unicode_stress_roundtrips(double max_time)
{
	...
	for(isize i = 0; i < MAX_ITERS; i++)
	{
        ...
        TEST_MSG(utf32 && utf16_cw && utf8_cw && utf32_cw && utf8_ccw && utf16_ccw && utf32_ccw, "OUT OF MEMORY!");
        ...
        TEST_MSG(finished_at == sequence_size, "Roundtrip conversion of valid codepoints must not fail!");

        isize utf8_cw_size = unicode_utf16_to_utf8(utf8_cw, max_utf8_size, utf16_cw, utf16_cw_size, &finished_at, UNICODE_ERROR);
        TEST_MSG(finished_at == utf16_cw_size, "Roundtrip conversion of valid codepoints must not fail!");

        isize utf32_cw_size = unicode_utf8_to_utf32(utf32_cw, max_utf32_size, utf8_cw, utf8_cw_size, &finished_at, UNICODE_ERROR);
        TEST_MSG(finished_at == utf8_cw_size, "Roundtrip conversion of valid codepoints must not fail!");
        ...
        TEST_MSG(finished_at == utf8_ccw_size, "Roundtrip conversion of valid codepoints must not fail!");

        isize utf32_ccw_size = unicode_utf16_to_utf32(utf32_ccw, max_utf32_size, utf16_ccw, utf8_ccw_size, &finished_at, UNICODE_ERROR);
        TEST_MSG(finished_at == utf16_ccw_size, "Roundtrip conversion of valid codepoints must not fail!");
        ...
        TEST_MSG(_unicode_are_memory_block_equal(utf8_cw, utf8_cw_size, utf8_ccw, utf8_ccw_size, sizeof(utf8_t)), "must match at all vertices!");
        TEST_MSG(_unicode_are_memory_block_equal(utf16_cw, utf16_cw_size, utf16_ccw, utf16_ccw_size, sizeof(utf16_t)), "must match at all vertices!");
        TEST_MSG(_unicode_are_memory_block_equal(utf32_cw, utf32_cw_size, utf32_ccw, utf32_ccw_size, sizeof(utf32_t)), "must match at all vertices!");

        //The original sequence must be perserved
        TEST_MSG(_unicode_are_memory_block_equal(utf32_cw, utf32_cw_size, utf32, sequence_size, sizeof(utf32_t)), "The original sequence must be perserved!");
        TEST_MSG(_unicode_are_memory_block_equal(utf32_from_16, utf32_from_16_size, utf32, sequence_size, sizeof(utf32_t)), "The original sequence must be perserved!");
        TEST_MSG(_unicode_are_memory_block_equal(utf32_from_8, utf32_from_8_size, utf32, sequence_size, sizeof(utf32_t)), "The original sequence must be perserved!");
