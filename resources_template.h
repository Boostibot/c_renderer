...
INTERNAL Type_Handle _resource_add(String name, String path, Resource_Info info, Storage_Type** out_ptr_or_null)
{
    ...
    _Resource* created = (_Resource*) handle_table_add(&resources_get()->resources[RESOURCE_TYPE], (Handle*) &obtained);
...
bool resource_remove(Type_Handle* handle)
{
    _Resource* prev = (_Resource*) handle_table_get(resources_get()->resources[RESOURCE_TYPE], (Handle*) &handle);
...
bool resource_force_remove(Type_Handle* handle)
{
    ...
    if(prev)
    {
        ...
        handle_table_remove(&resources_get()->resources[RESOURCE_TYPE], (Handle*) &handle);
    }
...
{
    Type_Handle out = {0};
    _Resource* item = (_Resource*) handle_table_get(resources_get()->resources[RESOURCE_TYPE], (Handle*) old_handle);
    if(item)
    {
